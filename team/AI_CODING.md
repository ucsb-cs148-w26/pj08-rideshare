...
I used ChatGPT (GPT-5) via the web interface to help implement and refine several features in our app, primarily around ride detail modals and cancellation flows. Specifically, I used it to design a driver-side cancellation feature that required a mandatory cancellation note and updated Firestore safely using a transaction. ChatGPT helped scaffold conditional rendering (driver vs. rider views), write a secure runTransaction block that validated ride ownership, and structure modal overlays cleanly without duplicating components. It was particularly helpful for reasoning through edge cases like ensuring only the ride owner could cancel, resetting local state correctly after cancellation, and modeling new fields such as status, cancelledAt, and cancellationNote. I also used it to debug a keyboard interaction issue where the cancellation note TextInput would trap the keyboard onscreen. ChatGPT suggested restructuring the modal using KeyboardAvoidingView, Pressable for dismissing the keyboard, ScrollView for scroll accessibility, and the updated submitBehavior API (since blurOnSubmit is deprecated). This resolved both usability and API deprecation issues efficiently.

While the AI significantly accelerated boilerplate-heavy UI work and transaction scaffolding, I did not accept outputs blindly. I tested every change in Expo, verified Firestore writes directly in the console, confirmed ownership checks prevented privilege escalation, and ensured UI state transitions behaved correctly. I also reviewed and simplified generated code to ensure I fully understood each pattern before integrating it. Going forward, I see AI as most useful for prototyping UI flows, debugging platform-specific quirks, and suggesting safer database patterns, but correctness, security validation, and architectural decisions still require hands-on testing and deliberate review to ensure reliability and fair use.